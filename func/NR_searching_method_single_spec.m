function [p2, interation_steps] = NR_searching_method_single_spec(p1, p2_initial, pcom_i, pcom_f, ...
...
% we assume the lateral velocity ends with 0.05m/s
while (i<=20 && abs(vcom_y_end(i))>=0.9e-4)
    
    pp2_initial(:,i) = [p2_initial(1); pp2(i); p2_initial(3)];
    [tcom_tmp, tcom_intersect_tmp, pcom_tmp, vcom_tmp, acom_tmp] = one_step_integration_single_spec(p1, pp2_initial(:,i), pcom_i, pcom_f,...
    vcomx_i, vcomx_f, acomx_i, acomx_f, vcomy_i, acomy_i, pcom_intersect, inc_x, inc_t);
    ...
    [tcom_tmp, tcom_intersect_tmp, pcom_tmp, vcom_tmp, acom_tmp] = one_step_integration_single_spec(p1, pp2_initial(:,i+1), pcom_i, pcom_f,...
    vcomx_i, vcomx_f, acomx_i, acomx_f, vcomy_i, acomy_i, pcom_intersect, inc_x, inc_t);
